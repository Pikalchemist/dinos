...
class Dataset(Module, SpaceManager):
    ...
    def __init__(self, modelClass=Model, options={}):
        ...
        """
        Module.__init__(self, 'Dataset')
        SpaceManager.__init__(self, storesData=True)
    ...
    def _serialize(self, options):
        ...
        dict_.update(Module._serialize(self, options))
        dict_.update(Serializer.serialize(
            self, ['models', 'options', 'iterationIds', 'iteration'], options=options))
        return dict_
    ...
    def addEvent(self, event, cost=1., p=None, p_type=None):
        ...
        event.id = eventId
        for point in event.actions.flat():
            #for action_part in action.get():
            idAction = point.space.addPoint(point, eventId, action=True)
            event.actionsRegister.append((point.space.id, idAction))
        for point in event.primitiveActions.flat():
            idAction = point.space.addPoint(point, eventId, action=True)
            event.primitiveActionsRegister.append((point.space.id, idAction))
        for point in event.outcomes.flat():
            idOutcome = point.space.addPoint(point, eventId, cost)
            event.outcomesRegister.append((point.space.id, idOutcome))
        if event.context:
            for point in event.context.flat():
                idContext = point.space.addPoint(point, eventId, cost)
                event.contextRegister.append((point.space.id, idContext))
